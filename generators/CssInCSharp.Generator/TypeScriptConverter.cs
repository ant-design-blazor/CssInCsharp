        ...
        private SyntaxNodeOrList GenerateCSharpAst(Ts.TsTypes.INode node, NodeContext? context = null)
        {
            switch (node.Kind)
            {
                case Ts.TsTypes.SyntaxKind.ArrowFunction:
                {
                    ...
                    switch (funcBody.Kind)
                    {
                        ...
                        case Ts.TsTypes.SyntaxKind.ParenthesizedExpression:
                            ...
                            {
                                case Ts.TsTypes.SyntaxKind.ObjectLiteralExpression:
                                    statements.Add(SyntaxFactory.ReturnStatement(GenerateCSharpAst(expression.Expression, new NodeContext(){ReturnType = returnType }).AsType<ExpressionSyntax>()));
                                    break;
                            }
                            break;
                    }
                    methodDeclaration = methodDeclaration.WithBody(SyntaxFactory.Block(statements));
                        ...
                        var variable = SyntaxFactory
                            ...
                            (
                                ...
                                (
                                    ...
                                    (
                                        SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, SyntaxFactory.IdentifierName(initializer), SyntaxFactory.IdentifierName(property))
                                    )
                                )
                            );
                ...
                case Ts.TsTypes.SyntaxKind.PropertyAssignment:
                {
                    ...
                    var field = GenerateCSharpAst(n.Name, context).AsType<ExpressionSyntax>();
                    if (n.Name.IsIndexerProperty())
                    {
                        field = SyntaxFactory.ElementAccessExpression(SyntaxFactory.IdentifierName(""))
                            .WithArgumentList(
                    ...
                    (
                        ...
                        field,
                        GenerateCSharpAst(initializer, context).AsType<ExpressionSyntax>()
                    );
                }
